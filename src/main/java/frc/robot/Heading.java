...
/**
 * Heading control utility class for the NavX/ADRS Gyro sensor.
 * 
...
public class Heading {
	...
	public double getHeading() {
		return angleToHeading(getNavXAngle());
	}
	...
	public void setHeading(double heading) {
		...
		pid.setTarget(getNavXAngle() + change);
	}
	...
	public void relTurn(double degrees){
		pid.setTarget(getNavXAngle() + degrees);
	}
	...
	public double turnRate() {
		pid.update();
		if (headingHold) {
			double turnRate = pid.calc(ahrs.getAngle());
			return turnRate;
		}
		else {
			return 0.0;
	...
	public double getDirection() {
		return (getNavXAngle() < pid.getTarget()) ? 1 : -1;
	}
}
